...
INTELLIGENT_VARIABLE_MODIFICATIONS_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            ...
            "Your modification requests will be structured into three sections: add, delete, and modify.\n\n"
            "**ADD:**\n"
            "- For each new decision variable, you MUST:\n"
            "  - Generate a valid JavaScript formula that references at least one existing or newly created raw indicator (or another decision variable).\n"
            "  - The formula must NOT be a placeholder or function name, but a real calculation (e.g., 'return avg_daily_customers * 30;').\n"
            "  - Provide a human-readable 'function' field that matches the formula.\n"
            "  - If you add a new raw indicator, include it in the 'new_variables' array with all required fields, and its 'formula' must be null.\n"
            "\n**DELETE:**\n"
            "- For each variable to delete, add its 'var_name' to the 'removed_variables' array.\n"
            "- Update or remove any formulas that depend on deleted variables.\n"
            "\n**MODIFY:**\n"
            "- For each variable to modify:\n"
            "  - Update both the 'formula' and 'function' fields to reflect the change.\n"
            "  - If dependencies change, update the 'raw_indicators' array.\n"
            "  - Output new formulas in 'updated_formulas' and new functions in 'updated_functions'.\n"
            "  - Do not leave the formula or function unchanged if the modification affects them.\n"
            "\n**IMPORTANT:**\n"
            "- Do NOT use placeholder strings or function names in formulas. All formulas must be valid JavaScript expressions referencing real variables.\n"
            "- For raw indicators, 'formula' must always be null.\n"
            "\n**EXAMPLES:**\n"
            "- Add: User says 'Add a new decision variable for sales growth.'\n"
            "  LLM invents a variable name, formula, and function, such as:\n"
            "    - var_name: monthly_sales_growth_rate\n"
            "    - formula: return (monthly_revenue - previous_month_revenue) / previous_month_revenue;\n"
            "    - function: monthly_sales_growth_rate = (monthly_revenue - previous_month_revenue) / previous_month_revenue\n"
            "- Add: User says 'Add another raw indicator for the decision variable customer_loyalty_score.'\n"
            "  LLM invents a new raw indicator (e.g., customer_repeat_rate), adds it to the raw_indicators list, and updates the formula for customer_loyalty_score to include the new indicator.\n"
            "- Delete: User says 'Remove the raw indicator for operating days.'\n"
            "  LLM removes the variable with var_name: operating_days and updates any dependent formulas.\n"
            "- Modify: User says 'Make the profit variable more granular.'\n"
            "  LLM splits the profit variable into subcomponents (e.g., gross_profit, net_profit), invents new variable names and formulas, and updates all related functions and dependencies.\n"
            ...
            "- updated_functions: Function field updates for decision variables.\n"
            "- updated_decision_variable_raw_indicators: Updated raw_indicators arrays for decision variables (mapping var_name to array of raw indicator var_names).\n"
            ...
            "- If the user does not specify details for the variable to add or modify, you must:\n"
            "  - Invent a realistic and relevant variable name, var_name, type, and description based on the business context and dependencies.\n"
            "  - For modifications, if asked to make a variable more granular or any other vague instruction, split or adapt the variable into logical subcomponents and update all related formulas and functions as needed.\n"
            "  - Never ask the user for clarification—always generate the most plausible and useful variable(s) for the context, regardless of the type of modification requested.\n"
        )
    ]
)
