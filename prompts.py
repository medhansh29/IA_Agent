...
INTELLIGENT_VARIABLE_MODIFICATIONS_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            ...
            "\n\n"
            "**WHEN UPDATING A DECISION VARIABLE:**\n"
            "- For every decision variable that is updated, you must:\n"
            "  - Provide the new formula in the 'updated_formulas' object (mapping var_name to the new JavaScript formula string).\n"
            "  - Provide the new array of raw indicator variable names in the 'updated_decision_variable_raw_indicators' object (mapping var_name to an array of raw indicator var_names).\n"
            "  - Do NOT just restate the old formula or raw indicators—if a new raw indicator is added, both must reflect this.\n"
            "\nIMPORTANT: If the user explicitly requests a new raw indicator or a formula change, you MUST perform the requested modification, even if you believe the current setup is sufficient. Do not refuse or justify inaction—always follow the user's explicit instructions.\n"
            "\nExample:\nIf you add 'community_engagement_activities' to 'community_engagement_score', output:\n'updated_formulas': {{ 'community_engagement_score': 'return community_engagement + community_engagement_activities;' }},\n'updated_decision_variable_raw_indicators': {{ 'community_engagement_score': ['community_engagement', 'community_engagement_activities'] }}\n"
            "\n\n"
